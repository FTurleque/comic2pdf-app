#Requires -Version 5.1
<#
.SYNOPSIS
    Lance les tests unitaires + couverture des outils CLI/watch (tools/).
.DESCRIPTION
    - Cree le venv si absent (py -3 ou python en fallback)
    - Installe les dependances dev (tools/requirements-dev.txt)
    - Lance pytest avec rapport de couverture terminal + XML + HTML
    - Impose un seuil minimal de couverture (PY_COV_MIN, defaut: 60%)
    Sorties : tools/coverage.xml
              tools/htmlcov/
.PARAMETER CovMin
    Seuil minimal de couverture (%). Defaut: 60 (env: PY_COV_MIN).
.EXAMPLE
    .\scripts\test_tools.ps1
    .\scripts\test_tools.ps1 -CovMin 70
#>
[CmdletBinding()]
param(
    [int]$CovMin = [int]($env:PY_COV_MIN -or 60)
)

Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"

$repoRoot = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path
$toolsDir = Join-Path $repoRoot "tools"
$testsDir = Join-Path $repoRoot "tests\tools"

Push-Location $toolsDir
try {
    # --- Creer le venv si absent ---
    if (-not (Test-Path ".venv")) {
        Write-Host "  > Creation du venv (.venv)..." -ForegroundColor Gray
        if (Get-Command py -ErrorAction SilentlyContinue) {
            py -3 -m venv .venv
        } else {
            python -m venv .venv
        }
    }

    $py = ".venv\Scripts\python.exe"

    # --- Dependances dev ---
    Write-Host "  > pip install -r requirements-dev.txt" -ForegroundColor Gray
    & $py -m pip install -q -r requirements-dev.txt
    if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

    # --- Tests + couverture (PYTHONPATH = racine du repo) ---
    Write-Host "  > pytest [tools] (seuil couverture: $CovMin%)" -ForegroundColor Cyan
    $env:PYTHONPATH = $repoRoot
    & $py -m pytest -q --tb=short `
        --cov=tools `
        --cov-report=term `
        --cov-report=term-missing `
        "--cov-report=xml:coverage.xml" `
        "--cov-report=html:htmlcov" `
        "--cov-fail-under=$CovMin" `
        $testsDir
    exit $LASTEXITCODE
}
finally {
    Pop-Location
}
