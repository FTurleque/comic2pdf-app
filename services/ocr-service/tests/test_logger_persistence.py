"""
Tests de persistance et rotation des logs pour l'ocr-service.
Vérifie :
...
def _reload_logger_module(monkeypatch, tmp_path, **env_overrides):
    ...
    monkeypatch.setenv("LOG_FILE", "ocr-service.log")
...
class TestOcrLoggerFileCreation:
    ...
    def test_fichier_log_cree(self, monkeypatch, tmp_path):
        ...
        logger = mod.get_logger("test_ocr_creation")
        try:
            logger.info("ocr message de test")
            log_file = tmp_path / "ocr-service.log"
    ...
    def test_deux_handlers(self, monkeypatch, tmp_path):
        ...
        logger = mod.get_logger("test_ocr_handlers")
        try:
...
class TestOcrLoggerRotation:

    def test_rotation_fichier_point_1_cree(self, monkeypatch, tmp_path):
        """Après dépassement de maxBytes, ocr-service.log.1 doit être créé."""
        ...
        logger = mod.get_logger("test_ocr_rotation")
        try:
            message = "O" * 80
            for _ in range(10):
                logger.info(message)
            ...
            rotated = tmp_path / "ocr-service.log.1"
            assert rotated.exists(), (
                f"Fichier rotatif ocr-service.log.1 absent. "
...
class TestOcrLoggerJsonFormat:
    ...
    def test_ligne_json_valide(self, monkeypatch, tmp_path):
        ...
        logger = mod.get_logger("test_ocr_json")
        try:
            logger.info("ocr json message")
            for h in logger.handlers:
                h.flush()
    ...
    def test_json_service_name(self, monkeypatch, tmp_path):
        """Le champ service doit valoir 'ocr-service' dans les logs JSON."""
        mod = _reload_logger_module(monkeypatch, tmp_path, LOG_JSON="true")

        logger = mod.get_logger("test_ocr_service_name")
        try:
...
class TestOcrResetLogger:
    ...
    def test_reset_supprime_tous_les_handlers(self, monkeypatch, tmp_path):
        ...
        logger = mod.get_logger("test_ocr_reset")
        assert len(logger.handlers) == 2
        mod.reset_logger_for_tests(logger)
